...
if __name__ == "__main__":
    ...
    if args.do_train:
        ...
        df.to_csv(f"/workspace/self-training/embeddings_variables/train_examples_{config['domain']}_{config['tmin']}.csv", index=False, encoding="utf-8-sig")
    ...
    df = pd.DataFrame(rows)
    df.to_csv(f"/workspace/self-training/embeddings_variables/train_features_{config['domain']}_{config['tmin']}.csv", index=False, encoding="utf-8-sig")
    ...
    if args.self_training:
            ...
            #domain_sim_df = pd.read_csv('/workspace/self-training/Compute-domain-simirlaty/results/domain_similarity_health_history.csv')
            ...
            df = pd.DataFrame(rows)
            df.to_csv(f"/workspace/self-training/embeddings_variables/self_training_examples_{config['domain']}_{config['tmin']}.csv", index=False, encoding="utf-8-sig")
            ...
            df = pd.DataFrame(rows)
            df.to_csv(f"/workspace/self-training/embeddings_variables/self_training_features_{config['domain']}_{config['tmin']}.csv", index=False, encoding="utf-8-sig")
    ...
    for run in range(5):
        ...
        df = pd.DataFrame(rows)
        df.to_csv(f"/workspace/self-training/embeddings_variables/selected_features_{config['domain']}_{config['tmin']}_dann-run{run}.csv", index=False, encoding="utf-8-sig")
        ...
        df = pd.DataFrame(rows)
        df.to_csv(f"/workspace/self-training/embeddings_variables/not_selected_features_{config['domain']}_{config['tmin']}_dann-run{run}.csv", index=False, encoding="utf-8-sig")
    ...
    if args.task_name == "ner":
        ...
        '/workspace/self-training/domain-Ad/self-fixedK/' : [f'base_model-self-Wojood-econ-{args.domain}-{args.tmin}.pt', f'model-self-Wojood-econ-{args.domain}-{args.tmin}.pt'],
