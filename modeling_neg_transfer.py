...
class TokenClassification(nn.Module):
    def __init__(self, pretrained_path, n_labels, hidden_size=768, dropout_p=0.1,
                 label_ignore_idx=0, head_init_range=0.04, device='cuda'):
        super().__init__()
    ...
    def forward(
        ...
        segments_mask=None,
        return_logits=False,
        return_sentence_losses=False,
        apply_source_masking=False,
        drop_p=10.0,
        eps=1e-12
    ):
        """
        x: [B,L,H]
        labels: [B,L]
        segments_mask: [B,L] (bool or 0/1) marks valid tokens
        """

        out_1 = self.dropout(x)
        logits = self.classification_head(out_1)  # [B,L,C]

        if labels is not None:
    # logits: [B,L,C], labels: [B,L]
            B, L, C = logits.shape
            if segments_mask is not None:
    # If dataloader gives list/np array, convert to tensor first
                if not torch.is_tensor(segments_mask):
                    segments_mask = torch.tensor(segments_mask)
                # Move to same device as labels (or logits)
                segments_mask = segments_mask.to(labels.device)
                
            if segments_mask is None:
                valid = (labels != self.label_ignore_idx)
            else:
                valid = segments_mask.bool() & (labels != self.label_ignore_idx)
            ...
            loss_tok = F.cross_entropy(
                ...
                labels,                   # [B,L]
                reduction="none",
                ignore_index=self.label_ignore_idx
            )

            v = valid.to(loss_tok.dtype)
            loss_tok = loss_tok * v

            # sentence loss ℓ_s -> [B]
            denom = torch.clamp(v.sum(dim=1), min=1.0)
            sent_loss = loss_tok.sum(dim=1) / (denom + eps)

            # default behavior (no sentence dropping)
            if not apply_source_masking:
                loss = sent_loss.mean()
                if return_sentence_losses:
                    return loss, sent_loss
                return loss

            # sentence masking: drop top drop_p% losses
            sent_loss_norm = self._minmax(sent_loss.detach(), eps=eps)
            cutoff = torch.quantile(sent_loss_norm, q=1.0 - drop_p / 100.0)
            m = (sent_loss_norm < cutoff).to(sent_loss.dtype)  # [B] 1 keep, 0 drop

            masked_loss = (m * sent_loss).sum() / (m.sum() + eps)

            if return_sentence_losses:
                return masked_loss, m, sent_loss
            return masked_loss
