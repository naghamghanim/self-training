...
from utils.train_utils import get_pseudo_labels_gtpl
...
class GTPLSequenceManager:
    ...
    def __init__(self, num_labels, base_threshold=0.9, alpha=0.8, device='cuda'):
        self.num_labels = num_labels
        self.base_thresh = base_threshold
        self.alpha = alpha  # Start with more weight on source domain
        self.device = device
        
        # Initialize thresholds per label ON THE DEVICE
        self.thresholds = torch.ones(num_labels, device=device) * base_threshold
        
        # Prototypes: track model confidence per label
        self.source_prototypes = torch.zeros(num_labels, device=device)  # From labeled data
        self.target_prototypes = torch.zeros(num_labels, device=device)  # From unlabeled data
        self.history_prototypes = torch.zeros(num_labels, device=device)  # EMA smoothed
        
        self.ema_decay = 0.9  # For smoothing prototypes
        self.update_count = 0
    
    def get_token_masks(self, probs, segments_mask):
        ...
        # Use thresholds already on device
        thresholds_expanded = self.thresholds[pred_labels]
...
if __name__ == "__main__":
    ...
    gtpl_manager = GTPLSequenceManager(
    num_labels=num_labels, 
    base_threshold=args.threshold,
    alpha=0.8  # Start with strong source influence
)
    ...
    for run in range(5):
        ...
        for i in range(num_iterations):
            if i == 0:
                ...
                selected_features_dann, _ = get_pseudo_labels_threshold(
                    base_network, 
                    classifier, 
                ...
                selected_features_dann, _ = get_pseudo_labels_gtpl(
                                                ...
                                                batch_size=args.eval_batch_size,
                                                device=device,
                                                with_segs=args.seg_true,num_labels=num_labels
                                            )    
            if i % test_interval == 0 and i > 0:
                print(f"Iteration {i}: Threshold range = {gtpl_manager.thresholds.min():.3f} - {gtpl_manager.thresholds.max():.3f}")
                print(f"  Alpha (source weight) = {gtpl_manager.alpha:.3f}")
            ...
            if i % len_train == 0:
                ...
                with torch.no_grad():
                    ...
                    # Get model predictions on source domain
                    bert_features, source_features = base_network(
                                                                    input_ids=tinput_ids, 
                                                                    attention_mask=tattention_mask, 
                                                                    segments=t_segments, 
                                                                    segments_mask=t_segments_mask,
                                                                    segments_indices_mask=t_segments_indices_mask,
                                                                    with_segs=args.seg_true, 
                                                                    is_source=True
                                                                )
            
                    logits = classifier(bert_features, labels=None)
                    probs = torch.nn.functional.softmax(logits, dim=2)
                    
                    # Update source prototypes in GTPL manager
                    gtpl_manager.update_source_prototypes(probs, labels_train, t_segments_mask, device=device)
            ...
            if not args.indomain:
                ...
                if run > 0:
                    with torch.no_grad():
                        ...
                        unlabeled_logits = classifier(unlabeled_features, labels=None)
                        unlabeled_probs = torch.nn.functional.softmax(unlabeled_logits, dim=2)
                    # Update target prototypes
                        gtpl_manager.update_target_prototypes(unlabeled_probs, u_segments_mask, device=device)
                        
                        # Get masks using dynamic thresholds
                        token_masks, pseudo_labels =  gtpl_manager.get_token_masks(unlabeled_probs, u_segments_mask)
                        
                        # Only compute loss for masked tokens
                        if token_masks.sum() > 0:
                            # Flatten for loss computation
                            logits_flat = unlabeled_logits.view(-1, num_labels)
                            labels_flat = pseudo_labels.view(-1)
                            masks_flat = token_masks.view(-1)
                            
                            # Compute loss only for high-confidence tokens
                            valid_logits = logits_flat[masks_flat]
                            valid_labels = labels_flat[masks_flat]
                            
                            if valid_logits.size(0) > 0:
                                pseudo_train_loss = F.cross_entropy(
                                    valid_logits, 
                                    valid_labels
                                )    

                        pseudo_train_loss = classifier(unlabeled_features, ulabels_train, u_segments_mask)
                        total_loss = total_loss + pseudo_train_loss * args.pseudo_weight
        ...
        # Use GTPL for pseudo-label generation instead of fixed threshold
    selected_features_dann, _ = get_pseudo_labels_gtpl(
            base_network, 
            classifier, 
            self_training_features, 
            ...
            device=device,
            with_segs=args.seg_true)
    print("Number of selected pseudo-labeled samples for DANN self-training: ", len(selected_features_dann))

    #after finished all self-training runs, do a final test on the test set
    if args.task_name == "ner":
        paths_for_best_models = {
        '/rep/nhamad/AdaSL/domain-Ad/self-fixedK/' : [f'base_model-self-PCMA-econ-{ran}.pt', f'model-self-PCMA-econ-{ran}.pt'],
