...
if __name__ == "__main__":
    ...
    if args.task_name == "ner": 
        test_datasets =  ['/var/home/nhamad/AdaSL/new-experiment/train-MSA'] # how may targets datasets
   
    val_dataloader = []
    for  val_data in test_datasets: # get valid.txt
    ...
    if args.do_train: # get train.txt 
    ...
    for run in range(5): # for self-training for 5 rounds
        ...
        if args.indomain: # no self-training
        ...
        if run > 0: #After Each run load the paramerters, updtae the paramters of the base_network 
            state_dict = torch.load(open(os.path.join(args.output_dir, f'base_model-Domain-{ran}.pt'), 'rb'))
        ...
        for i in range(num_iterations):
            ...
            if i % test_interval == 0 and i > 0: # finish training to calculate the F1-score / test_interval =500 after each 500 iteration to test on test datasets
                print("Start evaluation on dev (valid.txt) dataset")
                check_es = check_es + 500
                base_network.train(False)
                classifier.train(False)
                if args.task_name == "ner": # calculate F1-score on validation dataset  valid.txt file (Twitter-MSA, Twitter-DA)
                    ...
                    if temp_f1_msa > best_f1_msa:
                        ...
                        print("\n##########     save the best model domain.    #############\n")
                        log_str = "iter: {:05d}, Micro F1: {:.5f}".format(best_step, best_f1_msa)
                        print(log_str)
                        torch.save(base_network.state_dict(), open(os.path.join(args.output_dir, f'base_model-Domain-{ran}.pt'), 'wb'))
                        torch.save(classifier.state_dict(), open(os.path.join(args.output_dir, f'model-Domain-{ran}.pt'), 'wb'))
                    log_str = "iter: {:05d}, F1 domain: {:.5f}, best F1 for domain: {:.5f}".format(i, temp_f1_msa,best_f1_msa)
                    print(log_str)
        ...
        if args.task_name == "ner":
            ...
            state_dict = torch.load(open(os.path.join(args.output_dir, f'model-Domain-{ran}.pt'), 'rb'))
    ...
    if args.task_name == "ner":
        ...
        '/var/home/nhamad/AdaSL/new-experiment/train-MSA' : [f'base_model-Domain-{ran}.pt', f'model-Domain-{ran}.pt'],
