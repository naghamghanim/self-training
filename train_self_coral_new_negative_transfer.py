...
import modeling_neg_transfer as modeling
...
if __name__ == "__main__":
    ...
    for run in range(5):
        ...
        for i in range(num_iterations):
            ...
            tinput_ids, tattention_mask, labels_train, t_segments, t_segments_mask, t_segments_indices_mask = tinput_ids.to(device), tattention_mask.to(device), labels_train.to(device), t_segments.to(device), t_segments_mask.to(device), t_segments_indices_mask.to(device)

            if not torch.is_tensor(t_segments_mask):
                t_segments_mask = torch.tensor(t_segments_mask)
            t_segments_mask = t_segments_mask.to(device)
            bert_features, source_features = base_network(input_ids=tinput_ids, attention_mask=tattention_mask, segments = t_segments, segments_mask = t_segments_mask.to(device),segments_indices_mask= t_segments_indices_mask,with_segs = args.seg_true, is_source = True)
            
            #train_loss = classifier(bert_features, labels_train, t_segments_mask)
            train_loss, m_src, sent_loss = classifier(
                                                        bert_features,
                                                        labels=labels_train,
            ...
            #Lclass
            drop_ratio = 1.0 - (m_src.mean().item())
            avg_dropped_loss = sent_loss[m_src == 0].mean().item() if (m_src == 0).any() else 0.0
            ...
            optimizer.step()
            scheduler.step()
        print("for source batch:\n")
        print("Train loss: {:.4f}".format(train_loss.item()))
        print(f"Drop ratio: {drop_ratio*100:.2f}%, Avg dropped loss: {avg_dropped_loss:.4f}")
