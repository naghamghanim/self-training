...
if __name__ == "__main__":
    ...
    parser.add_argument('--unlabeled_data_dir', type=str, default='/home/ubuntu/self-training/CONLL-files/')
    ...
    if args.task_name == "ner": 
        ...
        test_datasets =  ['/home/ubuntu/self-training/CONLL-files/MSA', '/home/ubuntu/self-training/CONLL-files/DA']
    ...
    if args.do_train:
        ...
        df.to_csv("/home/ubuntu/self-training/embeddings/train_examples.csv", index=False, encoding="utf-8-sig")
    ...
    df = pd.DataFrame(rows)
    df.to_csv("/home/ubuntu/self-training/embeddings/train_features.csv", index=False, encoding="utf-8-sig")
    ...
    if args.self_training:
            ...
            df = pd.DataFrame(rows)
            df.to_csv("/home/ubuntu/self-training/embeddings/self_training_examples.csv", index=False, encoding="utf-8-sig")
            ...
            df = pd.DataFrame(rows)
            df.to_csv("/home/ubuntu/self-training/embeddings/self_training_features.csv", index=False, encoding="utf-8-sig")
    ...
    for run in range(5):
        ...
        if run > 0:
            if args.task_name == "ner":
                state_dict = torch.load(open(os.path.join(args.output_dir, f'base_model-self-PCMA-science-{ran}.pt'), 'rb'))
            if args.task_name == "pos":
            ...
            if i % test_interval == 0 and i > 0:
                ...
                if args.task_name == "ner":
                    ...
                    if temp_f1_msa > best_f1_msa:
                        ...
                        print("\n##########     save the best model self-PCMA-science.    #############\n")
                        ...
                        torch.save(base_network.state_dict(), open(os.path.join(args.output_dir, f'base_model-self-PCMA-science-{ran}.pt'), 'wb'))
                        torch.save(classifier.state_dict(), open(os.path.join(args.output_dir, f'model-self-PCMA-science-{ran}.pt'), 'wb'))
                    log_str = "iter: {:05d}, F1 self-PCMA-science: {:.5f}, best F1 for self-PCMA-science: {:.5f}".format(i, temp_f1_msa,best_f1_msa)
                    print(log_str)
                    print(metrics)
                    metrics_dict = vars(metrics)
                    print("Evaluation metrics")
        ...
        if args.task_name == "ner":
            ...
            state_dict = torch.load(open(os.path.join(args.output_dir, f'model-self-PCMA-science-{ran}.pt'), 'rb'))
        ...
        df = pd.DataFrame(rows)
        df.to_csv(f"/home/ubuntu/self-training/embeddings/selected_features_dann-run{run}.csv", index=False, encoding="utf-8-sig")
        ...
        df = pd.DataFrame(rows)
        df.to_csv(f"/home/ubuntu/self-training/embeddings/not_selected_features_dann-run{run}.csv", index=False, encoding="utf-8-sig")
    ...
    if args.task_name == "ner":
        ...
        '/home/ubuntu/self-training/domain-Ad/self-fixedK/' : [f'base_model-self-{ran}.pt', f'model-self-{ran}.pt'],
    ...
    for test_exp in test_datasets:
        ...
        base_path = os.path.join(args.output_dir, f'base_model-self-PCMA-science-{ran}.pt')
        clf_path  = os.path.join(args.output_dir, f'model-self-PCMA-science-{ran}.pt')
        ...
        if args.erroranalysis:
            ...
            eanalysis.to_csv(f'/home/ubuntu/self-training/error_analysis/{args.exp_msg}_{test_exp.replace("/","_").replace("-","_")}.csv',index=False)
            print(metrics)
            metrics_dict = vars(metrics)
