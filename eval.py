...
if __name__ == "__main__":
    ...
    if not args.test:
        for run in range(5):
            ...
            base_network = modeling.BertLayer(pretrained_path=args.pretrained_path)
            base_network = base_network.to(device)
            if not args.test: # only do training if not in test mode
                if run > 0:
                    if args.task_name == "ner":
    ...
    if args.task_name == "ner":
        ...
        '/workspace/self-training/domain-Ad/self-fixedK/' : [f'base_model-self-PCMA-science-{ran}.pt', f'model-self-PCMA-science-{ran}.pt'],
        
        }
    
    #after finished all self-training runs, do a final test on the test set
    ran="PCMA-science-X3SPTB"
    for test_exp in test_datasets:
        #test_exp="art-labeled"
        print("--------------------------------------")
        ...
        base_path = os.path.join(args.output_dir, f'base_model-self-{ran}.pt')
        clf_path  = os.path.join(args.output_dir, f'model-self-{ran}.pt')
