...
if __name__ == "__main__":
    ...
    label_list = data_processor.get_labels() # list of labels
    num_labels = len(label_list) + 1  # add one for IGNORE label = 9
    ...
    if args.task_name == "ner":
        test_datasets =  ['/var/home/nhamad/AdaSL/data/NER/Darwish-MSA', '/var/home/nhamad/AdaSL/data/NER/Darwish-DA'] # how may targets datasets
    ...
    val_dataloader = []
    for  val_data in test_datasets: # use target data
    ...
    if args.self_training: # if we want self-training put self-training= TRUE, AOC
    ...
    for run in range(5): # for self-training for 5 rounds
        print(f"run # of 5= {run}")
        if args.indomain:
            num_train_optimization_steps = int(
                (len(train_features)) / args.train_batch_size / args.gradient_accumulation_steps) * args.num_train_epochs
            num_iterations = (len(train_dataloader)) * args.num_train_epochs
        else:
            print("number of selected_features_dann")
        ...
        if run > 0: #After Each run load the paramerters, updtae the paramters of the base_network 
            if args.task_name == "ner":
        ...
        #train with training data (ANERCorp using base_netwok, classifier) Time: 
        for i in range(num_iterations):
            if i == 0:
                print(f"Training started with {device}")
            if i % test_interval == 0 and i > 0: # finish training to calculate the F1-score / test_interval =500 after each 500 iteration to test on test datasets
                print("Start evaluation on dev dataset")
                check_es = check_es + 500
                base_network.train(False)
                classifier.train(False)
                if args.task_name == "ner": # calculate F1-score on validation dataset (Twitter-MSA, Twitter-DA)
                    ...
                    print(log_str)
                if args.task_name == "pos":
                    temp_f1_egy, report_egy = sequence_labeling_test(val_dataloader[0], base_network, classifier,label_list,with_segs = args.seg_true)
                    temp_f1_glf, report_glf = sequence_labeling_test(val_dataloader[1], base_network, classifier,label_list,with_segs = args.seg_true)
                    temp_f1_lev, report_lev = sequence_labeling_test(val_dataloader[2], base_network, classifier,label_list,with_segs = args.seg_true)
                    temp_f1_mag, report_mag = sequence_labeling_test(val_dataloader[3], base_network, classifier,label_list,with_segs = args.seg_true)
                    
                    temp_f1_msa2, report_msa2 = sequence_labeling_test(val_dataloader[4], base_network, classifier,label_list,with_segs = args.seg_true)

                    if temp_f1_egy > best_f1_egy:
                        check_es = 0
                        best_step = i
                        best_f1_egy = temp_f1_egy
                        print("\n##########     save the best model EGY.    #############\n")
                        log_str = "iter: {:05d}, Macro F1: {:.5f}".format(best_step, best_f1_egy)
                        print(log_str)
                        torch.save(base_network.state_dict(), open(os.path.join(args.output_dir, f'base_model-EGY-{ran}.pt'), 'wb'))
                        torch.save(classifier.state_dict(), open(os.path.join(args.output_dir, f'model-EGY-{ran}.pt'), 'wb'))
                    log_str = "iter: {:05d}, F1 EGY: {:.5f}, best F1 for EGY: {:.5f}".format(i, temp_f1_egy,best_f1_egy)
                    print(log_str)

                    if temp_f1_glf > best_f1_glf:
                        check_es = 0
                        best_step = i
                        best_f1_glf = temp_f1_glf
                        print("\n##########     save the best model GLF.    #############\n")
                        log_str = "iter: {:05d}, Macro F1: {:.5f}".format(best_step, best_f1_glf)
                        print(log_str)
                        torch.save(base_network.state_dict(), open(os.path.join(args.output_dir, f'base_model-GLF-{ran}.pt'), 'wb'))
                        torch.save(classifier.state_dict(), open(os.path.join(args.output_dir, f'model-GLF-{ran}.pt'), 'wb'))
                    log_str = "iter: {:05d}, F1 GLF: {:.5f}, best F1 for GLF: {:.5f}".format(i, temp_f1_glf,best_f1_glf)
                    print(log_str)

                    if temp_f1_lev > best_f1_lev:
                        check_es = 0
                        best_step = i
                        best_f1_lev = temp_f1_lev
                        print("\n##########     save the best model LEV.    #############\n")
                        log_str = "iter: {:05d}, Macro F1: {:.5f}".format(best_step, best_f1_lev)
                        print(log_str)
                        torch.save(base_network.state_dict(), open(os.path.join(args.output_dir, f'base_model-LEV-{ran}.pt'), 'wb'))
                        torch.save(classifier.state_dict(), open(os.path.join(args.output_dir, f'model-LEV-{ran}.pt'), 'wb'))
                    log_str = "iter: {:05d}, F1 LEV: {:.5f}, best F1 for LEV: {:.5f}".format(i, temp_f1_lev,best_f1_lev)
                    print(log_str)
                    
                    
                    if temp_f1_mag > best_f1_mag:
                        check_es = 0
                        best_step = i
                        best_f1_mag = temp_f1_mag
                        print("\n##########     save the best model MAG.    #############\n")
                        log_str = "iter: {:05d}, Macro F1: {:.5f}".format(best_step, best_f1_lev)
                        print(log_str)
                        torch.save(base_network.state_dict(), open(os.path.join(args.output_dir, f'base_model-MAG-{ran}.pt'), 'wb'))
                        torch.save(classifier.state_dict(), open(os.path.join(args.output_dir, f'model-MAG-{ran}.pt'), 'wb'))
                    log_str = "iter: {:05d}, F1 MAG: {:.5f}, best F1 for MAG: {:.5f}".format(i, temp_f1_mag,best_f1_mag)
                    print(log_str)
                    
                    if temp_f1_msa2 > best_f1_msa2:
                        check_es = 0
                        best_step = i
                        best_f1_msa2 = temp_f1_msa2
                        print("\n##########     save the best model MSA.    #############\n")
                        log_str = "iter: {:05d}, Macro F1: {:.5f}".format(best_step, best_f1_msa2)
                        print(log_str)
                        torch.save(base_network.state_dict(), open(os.path.join(args.output_dir, f'base_model-MSA-{ran}.pt'), 'wb'))
                        torch.save(classifier.state_dict(), open(os.path.join(args.output_dir, f'model-MSA-{ran}.pt'), 'wb'))
                    log_str = "iter: {:05d}, F1 MSA: {:.5f}, best F1 for MSA: {:.5f}".format(i, temp_f1_msa2,best_f1_msa2)
                    print(log_str)
            ...
            if not args.indomain: # indomain=false ---> self-training ---> get unlabaled data
                if i % len_unlab == 0:
                    iter_unlab = iter(unlab_dataloader)
            
            #get the training data and put to device to start running    
            ...
            #pass the training features MSA to the base_network
            ...
            #pass ber_features to the classifier
            ...
            if not args.indomain:
                print("Calculate CORAL / self trianing")
            ...
            print("finish self training round")
            total_loss.backward()
            optimizer.step()
            scheduler.step()
    ...
    if args.task_name == "ner":
        ...
        '/var/home/nhamad/AdaSL/data/NER/Darwish-MSA' : [f'base_model-MSA-{ran}.pt', f'model-MSA-{ran}.pt'],
        '/var/home/nhamad/AdaSL/data/NER/Darwish-DA' : [f'base_model-DA-{ran}.pt', f'model-DA-{ran}.pt']
        }
    if args.task_name == "pos":
        paths_for_best_models = {
    ...
    #Finish training -- Start evalute using Twitter-MSA and Twitter-DA
